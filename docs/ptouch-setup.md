...
3. **テキスト** を追加（台帳の**機種名**を印刷）
   - オブジェクト名は `txtName` / `objName` を推奨（`テキスト1` でも可）
   - P-touch Editor でオブジェクトを選択 → プロパティの「名前」を確認
...
| 機種名テキスト | `txtName`（空欄でも可。未一致時はテンプレート内の最初のテキストオブジェクトを自動使用） |

機種名が印字されない場合は、テンプレートに**テキストオブジェクト**が含まれているか、バーコードだけの構成になっていないかを確認してください。
